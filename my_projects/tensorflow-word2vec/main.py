...
if __name__=='__main__':
    ...
    with open('stop_words.txt', encoding='utf-8') as f:
        ...
        while line:
            stop_words.append(line[:-1])  # 最后一行是换行符
            line = f.readline()  # 往下读
    ...
    raw_word_list = []
    sentence_list = []
    with open('fiction.txt', encoding='gbk') as f:
        line = f.readline()
        while line:
            while '\n' in line:
                line = line.replace('\n', '')  # 替换'\n'
            while ' ' in line:
                line = line.replace(' ', '')  # 去空
            if len(line) > 0:  # 如果句子非空
                raw_words = list(jieba.cut(line, cut_all=False))   # 分词
                dealed_words = []
                for word in raw_words:
                    if word not in stop_words and word not in ['qingkan520','www','com','http']:
                        raw_word_list.append(word)  # 处理后的所有词，用于词频统计
                        dealed_words.append(word)
                sentence_list.append(dealed_words)  # 处理后的句子，一行一个句子
            line = f.readline()
    word_count = collections.Counter(raw_word_list)  # 统计小说的词频，
    ...
    word_list = [x[0] for x in word_count]  # 拿到所有词，词频排前30000的词
    # 538777个单词,不重复单词数32738,选取前30000个单词进入词典
    # 创建模型，训练
    # 生成一个对象
    w2v = word2vec(vocab_list=word_list,    # 语料库
                   embedding_size=200,   # 向量大小
                   win_len=2,
                   learning_rate=1,
                   num_sampled=100,         # 负采样个数
                   logdir='D:/Pythoncode/deeplearning/NLP/tensorflow-word2vec/tmp/280')       # tensorboard 记录地址

    num_steps = 10000  # 迭代10000步
    for i in range(num_steps):
        #print (i%len(sentence_list))
        sent = sentence_list[i%len(sentence_list)]   # 一次取一个句子，重复多次
        w2v.train_by_sentence([sent])  # 训练
